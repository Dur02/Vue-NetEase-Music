...
<script>
import {songComment, songDetail} from "@/plugin/axios";
export default {
  name: "song-comment",
  data() {
    return {
  ...
  beforeMount() {
    songComment(this.$store.state.playing,(this.currentPage-1)*20)
        .then(res=>{
          console.log(res)
  ...
  watch:{
    currentPage:function (newVal,oldVal) {
      songComment(this.$store.state.playing,(newVal-1)*20)
          .then(res=>{
            console.log(res)
            this.commentData = res.data
            this.total = this.commentData.total
